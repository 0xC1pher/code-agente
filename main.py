...
class ClaudeAgent:
    def __init__(self, api_key=None, model="claude-3-opus-20240229", max_tokens=4000):
        ...
        self.system_prompt = """You are a helpful coding assistant with access to a text editor tool. You can view and modify files using the following commands:
...
2.  str_replace - Use this to modify file contents by replacing text.
    ...
    Important: The old_str MUST match EXACTLY with the content to replace (whitespace and indentation).
...
"""
    ...
    def chat(self):
        ...
        print(colored("=" * 10 + " Interactive Claude Agent with Text Editor Tool " + "=" * 10, "cyan"))
        print(colored("Type 'exit' to quit, 'history' to see conversation history.", "cyan"))
    ...
    def _process_message(self, user_message):
        ...
        try:
            # Initial Claude call with the user's message
            response = self.client.messages.create(
                model=self.model,
                max_tokens=self.max_tokens,
                system_prompt=self.system_prompt,
                messages=self.conversation_history,
                tools=[{"type": "text_editor_28258124", "name": "str_replace_editor"}],  # Add Tool
            )
            ...
            while response.stop_reason == "tool_use":
                ...
                # Process all tool use requests in the current response
                ...
                response = self.client.messages.create(
                    ...
                    system_prompt=self.system_prompt,
                    messages=self.conversation_history,
                    tools=[{"type": "text_editor_28258124", "name": "str_replace_editor"}],  # Keep Tool
                )
    ...
    def _handle_tool_use(self, tool_use):
        ...
        print(colored(f"\n🤖 In ⚙️ Using tool: {tool_use.name}", "yellow"))
    ...
    def _view_file(self, file_path, view_range=None):
        ...
        try:
            ...
            for i, line in enumerate(lines):
                line_num = start + i + 1  # Add line numbers
    ...
    def _replace_in_file(self, file_path, old_str, new_str):
        ...
        try:
            ...
            return f"Successfully replaced text at exactly one location in {file_path}"
    ...
    def _insert_in_file(self, file_path, insert_line, new_str):
        ...
        try:
            ...
            # Insert the new text
            lines.insert(insert_line, new_str.endswith('\n') and new_str or new_str + '\n')
    ...
    def _print_history(self):
        ...
        print(colored("=" * 10 + " Conversation History " + "=" * 10, "cyan"))
        for message in self.conversation_history:
            ...
            if role == "user":
                if content['type'] == "text":
                    print(colored(f"🧑‍💻 You: {content['text']}", "green"))
                elif content['type'] == "tool_result":
                    print(colored(f"[Tool Result: [ID: {content['tool_use_id']}] {content['content']}]", "yellow"))
