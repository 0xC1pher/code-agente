...
Debes asegurarte de que todas las acciones sean precisas y sigan los principios de código limpio y arquitectura limpia. Si los requerimientos del usuario son vagos, pide aclaraciones. En modo interactivo, confirma cambios significativos con el usuario. En modo autónomo, procede según el análisis y la planificación."""
    ...
    completed, pending = [], requirements[:]

    # Si no hay requerimientos claros, inferir tareas básicas de los archivos
    ...
    # Analizar completitud basada en archivos
    ...
    return {
        ...
        "completed": completed,
        "pending": [r["task"] for r in pending],
        "files": project_files
    }
    

    def _identify_technologies(self):
        """Identifica las tecnologías utilizadas en el proyecto."""
        tech_stack = []
        ...
        if os.path.exists(os.path.join(self.project_dir, "composer.json")):
            tech_stack.append("PHP (Composer)")
        if os.path.exists(os.path.join(self.project_dir, "Gemfile")):
            tech_stack.append("Ruby (Rails)")
        return tech_stack if tech_stack else ["Desconocido"]
    ...
    def _generate_project_status_response(self, project_analysis, tech_stack):
        """Genera una respuesta detallada sobre el estado del proyecto."""
        ...
        if project_analysis['completed']:
            response += "\n".join(f"  - {task}" for task in project_analysis['completed']) + "\n"
        else:
            response += "  - Ninguna\n"
        response += "- **Tareas pendientes:**\n"
        if project_analysis['pending']:
            response += "\n".join(f"  - {task}" for task in project_analysis['pending']) + "\n"
        else:
            response += "  - Ninguna\n"
        response += "- **Archivos presentes:**\n"
        if project_analysis['files']:
            response += "\n".join(f"  - {file}" for file in project_analysis['files']) + "\n"
        else:
            response += "  - Ninguno\n"
        if project_analysis['pending']:
            response += "\n**Sugerencia:** Considera trabajar en las tareas pendientes listadas arriba para avanzar."
        else:
            response += "\n**Felicidades:** El proyecto parece estar completo según los requerimientos."
        return response
...
"""    def _list_files(self, path, format_output=True):
        ...
        try:
            ...
            if format_output:
                return f"Archivos en '{path}':\n" + "\n".join(files)
            return files  # Retorna lista sin formato para uso interno
        except Exception as e:
            return f"Error al listar archivos: {str(e)}"""
